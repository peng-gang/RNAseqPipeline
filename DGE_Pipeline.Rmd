...
twoGroupCompare <- function(Feature_Counts,Sample_Info, Gene_Info, Variable_Of_Interest, Sample_Name, Groups_Selected, Covariates = NULL , Folder_name, pvalue_Cutoff, log2Fold_Cutoff){
...
formula<-as.formula(paste("~", Variable_Of_Interest))
  ...
  for(s in sigGN){
    ...
    gps[[i]] <- plotCt(dds, res, idx,Variable_Of_Interest, Sample_Name)
    i <- i + 1
  }
...
twoGroupCompare(Feature_Counts = cts,Sample_Info = sampleInfo,Gene_Info = geneInfo,Variable_Of_Interest = "group", Sample_Name='ID', Groups_Selected = c("KO_2","KO_4"),Folder_name = "KO_2VsKO_4", pvalue_Cutoff = 0.05,log2Fold_Cutoff = log2(1.5))
